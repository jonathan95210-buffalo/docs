...
Defining the model is normally is enjoyable and nothing changes with `wandb`.
...
The only difference is in the logging code. Previosly, you might have
reported metrics printing to the terminal, now you pass the same information to `run.log()`.
...
easier comparison across batch sizes, since this makes for easier comparison across batch sizes,
you can use raw steps or batch count. You can use raw steps or batch count. For longer training runs,
